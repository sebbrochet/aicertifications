# build-pandoc.ps1
# Builds the EPUB (and PDF if xelatex is present) of the "Copilot to Transformation" ebook
...
Write-Host "=== Pandoc builder — Copilot to Transformation ===" -ForegroundColor Cyan
Write-Host ""
...
# --- Metadata ---
$title    = "Copilot to Transformation"
$subtitle = "The complete guide to Microsoft's AI business certifications — AB-730 & AB-731"
...
if (-not $PdfOnly) {
    ...
    $epubOut = Join-Path $output 'Copilot-to-Transformation.epub'
...
if (-not $EpubOnly) {
    ...
    if (-not (Get-Command xelatex -ErrorAction SilentlyContinue)) {
        ...
        $pdfOut = Join-Path $output 'Copilot-to-Transformation.pdf'
        $header = Join-Path $buildDir 'latex-header.tex'
        # Full-bleed cover as PAGE 1 (mirrors the Copilot Studio Gems approach): eso-pic paints the
        # cover as the first page's background at \AtBeginDocument, and Pandoc's text title page is
        # suppressed — otherwise the title page is page 1 and the cover slips to page 2.
        $cover = Join-Path $assetsDir 'cover.png'
        $coverInclude = @()
        if (Test-Path $cover) {
